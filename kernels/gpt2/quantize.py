"""INT4 KV quantization helpers with uint8 nibble packing."""
...
def _dequantize_int4_kernel(
    ...
    out_ptrs_lo = out_ptr + pid_m * stride_om + out_cols_lo * stride_od
    out_ptrs_hi = out_ptr + pid_m * stride_om + out_cols_hi * stride_od

    tl.store(out_ptrs_lo, lo.to(tl.float16), mask=valid_pack)
    tl.store(out_ptrs_hi, hi.to(tl.float16), mask=valid_pack)
...
def _quantize_int4_triton(x: torch.Tensor, group_size: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    scales_2d = torch.empty((m, num_groups), device=x.device, dtype=torch.float16)
...
def quantize_int4(x: torch.Tensor, group_size: int = 32) -> tuple[torch.Tensor, torch.Tensor]:
    """Quantize last dimension to symmetric INT4 with per-group scales."""
    _validate_group_size(group_size)
    if x.dtype not in (torch.float16, torch.float32, torch.bfloat16):
        raise ValueError("quantize_int4 expects float16/float32/bfloat16 input")
    if x.dim() < 1:
...
def dequantize_int4(
    ...
    """Dequantize uint8-packed INT4 tensor back to float16."""
    _validate_group_size(group_size)
    if packed.dtype != torch.uint8:
        raise ValueError("packed must be uint8")
    if scales.dtype not in (torch.float16, torch.float32, torch.bfloat16):
        raise ValueError("scales must be float16/float32/bfloat16")
    ...
    if not packed.is_cuda or group_size % 2 != 0 or num_groups == 0:
        return _dequantize_int4_torch(packed, scales, group_size=group_size, d_head=target_d)
    return _dequantize_int4_triton(packed, scales, group_size=group_size, d_head=target_d)
...
def _dequantize_int4_torch(
    ...
    *,
    ...
    num_groups = scales.shape[-1]
    unpacked = torch.empty(*packed.shape[:-1], packed.shape[-1] * 2, device=packed.device, dtype=torch.float32)
    ...
    return out[..., :d_head].to(torch.float16)
...
def _dequantize_int4_triton(
    ...
    *,
    ...
    out_2d = torch.empty((m, padded_d), device=packed.device, dtype=torch.float16)
